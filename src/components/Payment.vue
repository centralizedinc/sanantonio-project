...
export default {
  ...
  methods: {
    selectMethod(method) {
      ...
      this.title =
        method === "CC" ? "Credit Card Payment" : "Over the Counter Payment";
    },
    submit() {
      if (this.payment_method === "CC") {
        if (this.mode === "permits") {
          this.details.progress.status = "PAID";
        } else {
        ...
        if (this.mode === "permits") {
          this.details.progress.status = "FOR PAYMENT";
        } else {
    ...
    success() {
      ...
      if (this.mode === "permits") {
        this.$store.dispatch("GET_PERMIT", this.details);
      } else {
